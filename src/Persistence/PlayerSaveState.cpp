...
PlayerSaveData PlayerSaveState::Capture(const Player &player)
{
    ...
    for (const auto &[questId, record] :
         player.GetQuestJournal().GetRecords())
    {
        saveData.quests.push_back({questId, record.state, record.progress});
    }
    ...
    bool validateEquipmentRequirements)
{
    ...
    if (saveData.version != CURRENT_PLAYER_SAVE_VERSION)
    {
        report.AddIssue(
            ...
            -1,
            "Only canonical player save version 3 is supported");
    }
    ...
    QuestJournal questValidationJournal;
    std::vector<QuestRecord> questRecords;
    questRecords.reserve(saveData.quests.size());
    for (const SavedQuestRecord &saved : saveData.quests)
        questRecords.push_back({saved.id, saved.state, saved.progress});
    if (!QuestSystem::TryRestore(questValidationJournal, questRecords))
        report.AddIssue(PlayerSaveValidationCode::INTERNAL_RESTORE_FAILURE,
            -1, "Invalid canonical quest records");
    ...
    PlayerSaveValidationReport &validationReport)
{
    ...
    std::vector<QuestRecord> restoredQuestRecords;
    restoredQuestRecords.reserve(saveData.quests.size());
    for (const SavedQuestRecord &saved : saveData.quests)
        restoredQuestRecords.push_back({saved.id, saved.state, saved.progress});
    if (!QuestSystem::TryRestore(
            player->GetQuestJournal(), restoredQuestRecords))
    {
        validationReport.AddIssue(
            PlayerSaveValidationCode::INTERNAL_RESTORE_FAILURE,
            -1, "Validated quest journal could not be restored");
        return nullptr;
    }
