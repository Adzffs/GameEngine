...
int main(){TestContext t;
 ...
 }
 {
  ...
  t.Expect(Result(w,accept)!=CommandResultCode::ACCEPTED&&Quest(*p).state==QuestState::AVAILABLE,"Dead Player rejects Accept through World queue");
  Restore(*p,QuestState::READY_TO_COMPLETE,10);
  auto complete=w.EnqueueCommand(QuestCompleteCommand{p->GetID(),1,sid,QuestId::GATHERING_BASICS});w.Update();
  t.Expect(Result(w,complete)!=CommandResultCode::ACCEPTED&&Quest(*p).state==QuestState::READY_TO_COMPLETE&&p->GetInventory().GetItemAmount(ItemType::COINS)==0,"Dead Player rejects Complete without reward");
 }
 {
  World w(40,41,42);auto*p=NewPlayer(w);Restore(*p,QuestState::READY_TO_COMPLETE,10);auto*s=Open(w,*p);auto sid=s->sessionId;
  auto reject=[&](const QuestCompleteCommand& command,const char* message){auto id=w.EnqueueCommand(command);w.Update();t.Expect(Result(w,id)!=CommandResultCode::ACCEPTED&&Quest(*p).state==QuestState::READY_TO_COMPLETE&&p->GetInventory().GetItemAmount(ItemType::COINS)==0,message);};
