...
std::string HistoricalSave(int version,const std::string& questRecord)
{
 std::string text="GAMEENGINE_PLAYER_SAVE\nversion="+std::to_string(version)+"\nposition_x=3\nposition_y=2\ncurrent_health=100\ninventory_count=28\n";
 for(int index=0;index<28;++index)text+="inventory."+std::to_string(index)+"=NONE,0\n";
 text+="skill_count=5\nskill.ATTACK=0\nskill.DEFENCE=0\nskill.WOODCUTTING=0\nskill.MINING=0\nskill.SMITHING=0\n";
 text+="equipment_count=5\nequipment.HEAD=NONE\nequipment.BODY=NONE\nequipment.LEGS=NONE\nequipment.WEAPON=NONE\nequipment.SHIELD=NONE\n";
 if(version==3)text+="quest_count=1\n";
 text+=questRecord;
 text+="END_GAMEENGINE_PLAYER_SAVE\n";
 return text;
}
}
...
int main(){
 ...
 const std::string v2=HistoricalSave(2,"quest.GATHERING_BASICS=ACTIVE,2\n");auto decodedV2=PlayerSaveTextCodec::Decode(v2);t.Expect(decodedV2.IsSuccess()&&decodedV2.GetSaveData()->version==4&&decodedV2.GetSaveData()->quests.size()==2&&decodedV2.GetSaveData()->quests[0].state==QuestState::ACTIVE&&decodedV2.GetSaveData()->quests[0].progress==2&&decodedV2.GetSaveData()->quests[1].state==QuestState::AVAILABLE&&decodedV2.GetSaveData()->quests[1].progress==0,"Genuine version 2 Gathering record migrates to current in memory");
 const auto invalidV2Mining=PlayerSaveTextCodec::Decode(HistoricalSave(2,"quest.MINING_BASICS=ACTIVE,3\n"));t.Expect(!invalidV2Mining.IsSuccess()&&!invalidV2Mining.GetSaveData().has_value(),"Version 2 Mining-only record rejects without exposing destination state");
 for(const auto& [token,state,progress]:std::vector<std::tuple<std::string,QuestState,int>>{{"UNAVAILABLE,0",QuestState::UNAVAILABLE,0},{"AVAILABLE,0",QuestState::AVAILABLE,0},{"ACTIVE,6",QuestState::ACTIVE,6},{"READY_TO_COMPLETE,10",QuestState::READY_TO_COMPLETE,10},{"COMPLETED,10",QuestState::COMPLETED,10}}){const auto result=PlayerSaveTextCodec::Decode(HistoricalSave(3,"quest.GATHERING_BASICS="+token+"\n"));const auto* data=result.GetSaveData()?&*result.GetSaveData():nullptr;t.Expect(result.IsSuccess()&&data&&data->version==4&&data->quests.size()==2&&data->quests[0].id==QuestId::GATHERING_BASICS&&data->quests[0].state==state&&data->quests[0].progress==progress&&data->quests[1].id==QuestId::MINING_BASICS&&data->quests[1].state==QuestState::AVAILABLE&&data->quests[1].progress==0,"Every genuine version 3 Gathering state migrates exactly and initializes Mining");if(state==QuestState::COMPLETED&&data){auto migratedPlayer=PlayerSaveState::TryCreatePlayer(99,*data,report);t.Expect(migratedPlayer&&!QuestSystem::Complete(migratedPlayer->GetQuestJournal(),QuestId::GATHERING_BASICS,migratedPlayer->GetInventory())&&migratedPlayer->GetInventory().GetItemAmount(ItemType::COINS)==0,"Version 3 completed Gathering remains unrewardable");}}
 const auto completedV2=PlayerSaveTextCodec::Decode(HistoricalSave(2,"quest.GATHERING_BASICS=COMPLETED,10\n"));auto migratedPlayer=completedV2.IsSuccess()?PlayerSaveState::TryCreatePlayer(98,*completedV2.GetSaveData(),report):nullptr;t.Expect(migratedPlayer&&!QuestSystem::Complete(migratedPlayer->GetQuestJournal(),QuestId::GATHERING_BASICS,migratedPlayer->GetInventory())&&migratedPlayer->GetInventory().GetItemAmount(ItemType::COINS)==0,"Version 2 completed migration cannot reward again");
 const auto migrated=PlayerSaveTextCodec::Decode(HistoricalSave(1,""));t.Expect(migrated.IsSuccess()&&migrated.GetSaveData()->version==4&&migrated.GetSaveData()->quests.size()==2&&migrated.GetSaveData()->quests[0].state==QuestState::AVAILABLE&&migrated.GetSaveData()->quests[1].state==QuestState::AVAILABLE,"Version 1 initializes both canonical quests");
